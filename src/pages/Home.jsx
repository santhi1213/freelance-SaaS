...
import { FaMoneyBillWave, FaCode, FaClock, FaFilter, FaStar, FaSearch, FaChevronDown, FaChevronUp, FaCalendarAlt, FaArrowRight, FaSpinner } from 'react-icons/fa';
  ...
  const fetchProjects = async () => {
    try {
      ...
      const transformedProjects = data.map(project => ({
        ...
        client: {
          name: project.email.split('@')[0], // Extract name from email
          email: project.email,
          rating: 4.5, // Default rating since not provided by API
          profile: `https://ui-avatars.com/api/?name=${project.email.split('@')[0]}&background=3B82F6&color=ffffff&size=50`
        },
      ...
      // Set featured projects (high-value projects)
      const highValueProjects = transformedProjects
        .filter(project => project.budget_from >= 1000)
        .sort(() => 0.5 - Math.random())
        .slice(0, 3);
      
      setFeaturedProjects(highValueProjects.length > 0 ? highValueProjects : transformedProjects.slice(0, 3));
