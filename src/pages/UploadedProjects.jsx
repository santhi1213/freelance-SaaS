...
const UploadedProjects = ({ darkMode }) => {
  ...
  // Helper function to map API bid status to display status
    ...
    try {
      ...
      if (data.success) {
        ...
        const transformedProjects = data.data.map(project => ({
          ...
          title: project.title,
          description: project.description,
          status: mapProjectStatus(project.status), // Use mapped status
          budget: `${project.budget_from || 0} - ${project.budget_to || 0}`,
          ...
          bids: project.bids ? project.bids.map(bid => ({
            ...bid,
            status: mapBidStatus(bid.status), // Map bid status for display
            freelancerName: bid.freelancer?.fullName || 'Unknown Freelancer',
  ...
  //           year: 'numeric'
  //         }),
  //         status: mapBidStatus(bid.status), // Use mapped status
  //         bidAmount: `${bid.amount.toLocaleString()}`,
  //         proposedDuration: bid.deliveryTime,
  //         coverLetter: bid.coverLetter,
  //         createdAt: bid.createdAt,
  ...
  //   }
  // };
  // Replace the fetchMyBids function with this corrected version:
...
const fetchMyBids = async () => {
  ...
  try {
    ...
    if (data.message === "Bids fetched successfully" && data.bids) { // Changed condition
      // Transform API response to match the component's expected format
      const transformedBids = data.bids.map(bid => ({
        id: bid._id, // Changed from bid.project._id to bid._id
        projectTitle: bid.project.title,
        projectDescription: bid.project.description,
        projectId: bid.project._id,
        client: bid.project.clientName,
        ...
        coverLetter: bid.coverLetter,
        createdAt: bid.createdAt,
        clientId: bid.project.clientEmail || bid.project.email // Added fallback
      }));
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 w-[1200px]">
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        ...
                        <tbody>
                          {filteredProjects.map(bid => (
                            // <tr key={bid.id} className="dark:border-gray-700 bg-white/50 hover:bg-gray-10 dark:bg-gray-700 dark:hover:bg-gray-600">
                            <tr
  key={bid.id}
  className={`${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-100'} dark:border-gray-700`}
>
                          ...
                          <div className="space-y-3 max-h-60 overflow-y-auto">
                            {project.bids.map((bid, index) => (
                              <div key={index} className={`p-4 border rounded-lg ${darkMode ? 'border-gray-600' : 'border-gray-200'}`}>
                                <div className="flex justify-between items-start mb-2">
                                  <div>
                                    <p className="font-medium">{bid.freelancer?.fullName || 'Freelancer'}</p>
                                    <p className="text-sm text-gray-500">Bid Amount: ${bid.amount}</p>
                                    <p className="text-sm text-gray-500">Delivery Time: {bid.deliveryTime}</p>
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <button
                                      onClick={() => startConversation(bid.freelancer?.id, project.id)}
                                      className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm"
                                    >
                                      Message
                                    </button>
