...
const ProfilePage = ({ darkMode }) => {
  ...
  const handlePhotoSelect = (e) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setPopup({show:true, type:'error', message:'Please select a valid image file (JPEG, PNG, GIF, WebP)'})
        return;
      }

      // Validate file size (5MB)
      if (file.size > 5 * 1024 * 1024) {
        setPopup({show:true, type:'error', message:'File size must be less than 5MB'})
        return;
      }
  ...
  const handlePhotoUpload = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setPopup({show:true, type:'success', message:'Profile photo updated successfully!'})
      } else {
        throw new Error(result.message || 'Failed to upload photo');
      }
    } catch (error) {
      console.error('Error uploading photo:', error);
      setPopup({show:true, type:'error', message:'Failed to upload photo: ' + error.message})
    } finally {
  ...
  const handlePhotoDelete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setPopup({show: true, type:'success', message:'Profile photo deleted successfully!'})
      } else {
        throw new Error('Failed to delete photo');
      }
    } catch (error) {
      console.error('Error deleting photo:', error);
      setPopup({show:true, type:'error', message:'Failed to deleted photo: ' + error.message})
    }
  };
  ...
  const updateUserProfile = async (updatedData) => {
    try {
      ...
      setPopup({ show: true, type: 'error', message: 'Failed to update profile: ' + err.message})
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="container mx-auto px-4">
                ...
                {isEditing ? (
                  <div className="space-y-3">
                    <input
                      ...
                      className={`block w-full text-2xl font-bold px-3 py-2 mt-4 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                        }`}
                    />
                    <input
                      type="text"
                      ...
                      className={`block w-full text-lg text-gray-600 dark:text-gray-300 px-3 py-2 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'
                        }`}
                    />
                    <div className="flex items-center mt-2">
                      ...
                      <input
                        ...
                        className={`w-full text-sm px-3 py-2 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600 text-gray-300' : 'bg-white border-gray-300 text-gray-600'
                          }`}
                      />
                    </div>
                  </div>
              ...
              <div className={`px-4 py-2 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                ...
                {isEditing ? (
                  <select
                    ...
                    onChange={handleFormChange}
                    className={`px-2 py-1 rounded border text-sm ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                      }`}
                  >
        ...
        <div className="flex overflow-x-auto mb-8 border-b border-gray-200 dark:border-gray-700">
          ...
          <button
            className={`px-4 py-2 font-medium whitespace-nowrap ${activeTab === 'portfolio'
                ? 'text-blue-600 border-b-2 border-blue-600'
                : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
              }`}
          ...
          <button
            className={`px-4 py-2 font-medium whitespace-nowrap ${activeTab === 'reviews'
                ? 'text-blue-600 border-b-2 border-blue-600'
                : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
              }`}
          ...
          <button
            className={`px-4 py-2 font-medium whitespace-nowrap ${activeTab === 'settings'
                ? 'text-blue-600 border-b-2 border-blue-600'
                : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
              }`}
                    ...
                    {isEditing && (
                      <div className="flex">
                        <input
                          ...
                          className={`px-3 py-1 rounded-l-lg border ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                            }`}
                        />
                        <button
                    ...
                    {isEditing && (
                      <div className="mt-6 p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                        ...
                        <textarea
                          ...
                          rows={3}
                          className={`w-full mt-3 px-3 py-2 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'
                            }`}
                        />
                        <button
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            {project.technologies && project.technologies.length > 0 ? (
                              project.technologies.map((tech, techIndex) => (
                                <span
                                  key={techIndex}
                                  className={`text-xs px-2 py-1 rounded-full ${darkMode ? 'bg-blue-900 bg-opacity-20 text-blue-300' : 'bg-blue-100 text-blue-800'
                                    }`}
                                >
                                  {tech}
                                </span>
