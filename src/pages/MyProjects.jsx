...
//     setError(null);
//     try {
//       // Get the email from localStorage (or auth context)
//       const email = localStorage.getItem('email');
//       // Replace with your backend base URL
//       const response = await fetch(`https://freelance-backend-0tw4.onrender.com/api/projects/accepted-by-email/${email}`, {
//         method: 'GET',
//         headers: getHeaders()
//       });
...
//           },
//           createdAt: project.createdAt,
//           canStartChat: false, // You can set this if required
//           milestones: [], // For modal; can be filled if your API supports it
//         }));
//         console.log(transformedProjects);

//         setMyProjects(transformedProjects);
//         // If you want to update pagination: setPagination(data.pagination);
//       }
//     } catch (err) {
//       setError(err.message);
...
//     }
//   };
//   const fetchProjectsWithCompletion = async () => {
//     try {
//       const updatedProjects = await Promise.all(myProjects.map(async (project) => {
//         try {
//           const completionRes = await fetch(`http://localhost:5000/api/projects/${project.id}/completion`);
//           const completionData = await completionRes.json();
//           return { ...project, progress: completionData.project?.completionPercentage || 0 };
//         } catch {
...
//   // useEffect(() => {
//   //   if (activeTab === 'active' && role === 'freelancer') {
//   //     fetchAcceptedProjects();
//   //     fetchProjectsWithCompletion();
//   //   } else {
//   //     fetchMyProjects();
//   //   }
...
//   useEffect(() => {
//     if (activeTab === 'active' && role === 'freelancer') {
//       fetchAcceptedProjects().then(() => fetchProjectsWithCompletion());
//       fetchMyProjects().then(() => fetchProjectsWithCompletion());
//     } else if (activeTab === 'active') {
//       fetchMyProjects().then(() => fetchProjectsWithCompletion());
//     }
//     if (activeTab === 'proposals') fetchMyBids();
//   }, [activeTab]);
...
//           <button
//             className={`py-2 px-4 text-center ${activeTab === 'active'
...
//           {role === 'freelancer' && (
//             <button
//               className={`py-2 px-4 text-center ${activeTab === 'proposals'
//                 ? 'border-b-2 border-blue-600 font-medium text-blue-600'
//                 : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
//                 }`}
...
//                             <tr key={bid.id} className={`bg-white hover:bg-gray-100 ${darkMode ? 'dark:bg-gray-800 dark:hover:bg-gray-700' : ''} `}>
//                               <td className="py-4 px-4">
//                                 <div>
//                                   <div className="font-medium uppercase">{bid.projectTitle}</div>
//                                   <div className={`text-xs mt-1 text-black ${darkMode ? 'text-white' : ''} line-clamp-2`}>
...
const MyProjectsPage = ({ darkMode }) => {
    ...
    try {
      ...
      if (data.success) {
        const transformedProjects = data.data.map(project => ({
          ...
          progress: 0, // Will be updated by fetchProjectsWithCompletion
          duration: project.project_duration || 'Not specified',
          category: project.project_type || 'General',
          skills: project.req_skills || [],
          client: {
    ...
    try {
      ...
      if (data.success) {
        const transformedProjects = data.data.map(project => ({
          ...
          createdAt: project.createdAt,
          canStartChat: false,
          milestones: [], // Add empty milestones array for modal
        }));
        console.log('Transformed accepted projects:', transformedProjects);
  ...
  // Fixed fetchProjectsWithCompletion function
  const fetchProjectsWithCompletion = async () => {
    if (myProjects.length === 0) return;
    
    console.log('Fetching completion for projects:', myProjects.length);
  ...
  useEffect(() => {
    console.log('useEffect triggered, activeTab:', activeTab, 'role:', role);
    ...
    fetchData();
  }, [activeTab, role, userId]); // Added proper dependencies

  // Separate useEffect for fetching completion data
  useEffect(() => {
    if (activeTab === 'active' && myProjects.length > 0) {
      console.log('Fetching completion data for projects...');
      fetchProjectsWithCompletion();
    }
  }, [myProjects.length > 0 ? myProjects.map(p => p.id).join(',') : '']); // Only trigger when project IDs change
  ...
  const filteredProjects = (activeTab === 'active' ? myProjects : myBids)
    ...
    .sort((a, b) => {
      if (activeTab === 'active') {
        switch (filters.sortBy) {
          case 'deadline':
            if (a.deadline === 'No deadline set' && b.deadline === 'No deadline set') return 0;
            if (a.deadline === 'No deadline set') return 1;
            if (b.deadline === 'No deadline set') return -1;
            ...
            {activeTab === 'active' && (
              <>
                {loading ? (
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
                    <p className={darkMode ? 'text-gray-300' : 'text-gray-600'}>Loading your projects...</p>
                  </div>
                ) : error ? (
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex items-center mt-2">
                            <img
                              ...
                              className="w-8 h-8 rounded-full mr-2"
                              onError={(e) => {
                                e.target.src = 'data:image/svg+xml;base64,';
                              }}
                            />
                            <div>
                              <p className="text-sm font-medium">{project.client.name}</p>
            ...
            {activeTab === 'proposals' && (
              <>
                {loading ? (
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
                    <p className="text-red-500 mb-4">You haven't placed any Bids</p>
                    <button
                      onClick={fetchMyBids}
                      className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg"
                    >
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 w-full max-w-none">
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead className={`text-left ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} dark:border-gray-700`}>
                          <tr>
                      ...
                      <div className={`p-4 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} mb-6`}>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <div className="flex items-center">
                              <FaRegClock className="mr-1 text-purple-500" />
                        ...
                        <div className="mb-6">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {project.skills.map((skill, index) => (
                              <span
                                key={index}
                                className={`px-3 py-1 text-xs rounded-full ${darkMode ? 'bg-blue-900 bg-opacity-20 text-blue-300' : 'bg-blue-100 text-blue-800'}`}
                              >
                                {skill}
                              </span>
                            ))}
