      ...
      setSuccessMessage(true);
      {
        (successMessage || errorMessage) && (
          <div className="fixed bottom-6 right-6 z-[60]">
            <div
              className={`px-6 py-4 rounded-lg shadow-lg flex items-center space-x-3 transition-all duration-300 ${successMessage
                  ? 'bg-green-600 text-white'
                  : 'bg-red-600 text-white'
                }`}
            >
              {successMessage ? <FaCheck /> : <FaExclamationTriangle />}
              <span className="text-sm font-medium">
                {successMessage ? 'Project posted successfully!' : errorMessage}
              </span>
            </div>
          </div>
        )
      ...
      <div className={`w-full max-w-3xl ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-800'} rounded-xl shadow-2xl overflow-hidden`}>
        ...
        <div className="flex justify-between px-8 py-3 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center space-x-4">
            <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${currentStep >= 1
          ...
          <div className={`flex items-center ${currentStep >= 2 ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400'}`}>
            ...
            <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${currentStep >= 2
                ? 'bg-blue-600 text-white'
                : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'
              }`}>
              2
          ...
          <div className={`flex items-center ${currentStep >= 3 ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400'}`}>
            <FaChevronRight size={14} className="mx-2" />
            <div className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${currentStep >= 3
                ? 'bg-blue-600 text-white'
                : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'
              }`}>
              3
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          onChange={handleChange}
                          placeholder="Min"
                          className={`w-full pl-7 px-4 py-3 rounded-lg border ${darkMode
                              ? 'bg-gray-800 border-gray-700 focus:border-blue-500 text-white'
                              : 'bg-gray-50 border-gray-200 focus:border-blue-500'
                            } focus:ring-2 focus:ring-blue-200 outline-none transition-all`}
                        />
              ...
              {currentStep === 3 && (
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {formData.skills.map((skill, index) => (
                        <span
                          key={index}
                          className={`px-3 py-1.5 rounded-full text-sm font-medium flex items-center ${darkMode ? 'bg-blue-900 text-blue-200' : 'bg-blue-100 text-blue-800'
                            }`}
                        >
                          {skill}
                          <button
                    ...
                    <div className="flex mb-4">
                      <input
                        ...
                        className={`flex-1 px-4 py-2 rounded-l-lg border-y border-l ${darkMode
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-r-lg ${customSkill
                    ...
                    <div className="flex flex-wrap gap-2">
                      {popularSkills.map((skill, index) => (
                        <button
                          key={index}
                          onClick={() => handleSkillToggle(skill)}
                          className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${formData.skills.includes(skill)
                  ...
                  <div>
                    ...
                    <div className={`border-2 border-dashed rounded-lg p-6 text-center ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-300 bg-gray-50'
                      }`}>
                      ...
                      <label
                        htmlFor="file-upload"
                        className={`inline-block px-4 py-2 rounded-lg cursor-pointer ${darkMode
                    ...
                    {formData.attachments.length > 0 && (
                      <div className="mt-4 space-y-2">
                        ...
                        {formData.attachments.map((file, index) => (
                          <div
                            key={index}
                            className={`flex items-center justify-between p-2 rounded ${darkMode ? 'bg-gray-800' : 'bg-gray-100'
                              }`}
                          >
          ...
          <div className={`px-6 py-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} flex justify-between`}>
            {currentStep > 1 ? (
              <button
                ...
                className={`px-5 py-2 rounded-lg ${isSubmitting
            ...
            {currentStep < 3 ? (
              <button
                ...
                className={`px-5 py-2 rounded-lg ${isStepValid() && !isSubmitting
                    ? 'bg-blue-600 hover:bg-blue-700 text-white'
                    : 'bg-gray-300 dark:bg-gray-700 text-gray-500 dark:text-gray-400 cursor-not-allowed'
                  }`}
              >
